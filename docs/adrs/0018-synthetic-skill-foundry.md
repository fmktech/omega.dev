...
The daemon validates and content-addresses all fixture files, public objectives, hidden executable verifiers, static diagnostics, negative invariants, environment descriptions, and provenance before execution. Unsafe paths, duplicate or missing variations, missing executable verifiers, empty invariants, and oversized fixtures fail closed. Static source predicates are diagnostics only: they cannot establish baseline redness or post-task capability.

Proposal synthesis and workspace execution have separate budgets. `EvolutionRequest.budget` bounds the two proposal-only children; the daemon's `benchmarks.syntheticSkillTaskBudget` independently bounds each generated workspace trial. A cheap reflection request therefore cannot silently make its promotion test incapable of ordinary discovery, editing, and verification.

The trusted benchmark launcher then runs the incumbent and candidate on identical copies of all three fixtures. Task runners receive the objective and fixture, never the hidden verifier or applicability expectation. After the runner terminates, the launcher mirrors the completed workspace into a private verifier directory, overlays non-colliding verifier files, and runs the verifier in an offline, read-only OCI sandbox. Verifiers address the mirrored task tree by workspace-relative paths such as `./src/module.js`; verifier files may not replace candidate files. Runtime behavior is authoritative, while static predicates remain report diagnostics. Positive candidate runs must load the candidate-only skill component. The negative-control candidate run must not load that component. Reads of unrelated incumbent skills neither satisfy nor violate these gates.
...
- Hidden executable verifiers accept behaviorally equivalent implementations and reject source-text mimicry without exposing their assertions to the task-solving runner.
