...
An evolution child may return either one direct component delta or an evidence-grounded reflection proposal. For a skill-scoped reflection, the daemon atomically compiles the skill and every supported related knowledge, runner, tool, and policy lesson into one canonical project bundle rooted at `skills/<name>/SKILL.md`. Each document records the source session, evidence artifacts, proposal artifact, cited evidence references, and evidence digest. A repeated lesson with the same semantic digest is a no-op; revised guidance replaces the installed skill with the same canonical entrypoint, preserving harness parent lineage rather than growing duplicate skills. A valid `no-change` reflection terminates as rejected without running a promotion evaluation.

A skill-only `harness.evolve` request defaults to the synthetic skill foundry. The daemon launches an evolution child and an independent `promotion-eval` child from the same explicitly supplied evidence artifacts. The latter returns exactly one near-transfer, generalization, and negative-control workspace fixture; the daemon validates and content-addresses public fixture data separately from hidden checks and invariants. The trusted launcher runs incumbent and candidate on all three fixtures. Positive candidate runs must load a candidate-only skill component, while the negative-control candidate run must not load it. Only the resulting promotable scorecard activates the project candidate. Continuation artifacts are readable through `artifact.read`; other cross-session artifacts remain denied.
