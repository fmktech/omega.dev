...
export function renderReflectionPrompt(scenario: ReflectionScenario): Result<{ readonly prompt: string; readonly evidenceSha: Sha256 }, EvolutionError> {
  ...
  const prompt = [
    ...
    "A tool output from one successful run is observational evidence, not automatically a durable output requirement. Treat an exact value as normative only when the user, repository specification, or authoritative contract explicitly requires it. A historical success message is not permission to rewrite an existing verifier or tool to reproduce that message; preserve the current verifier/tool and run it as-is unless the task explicitly changes it.",
