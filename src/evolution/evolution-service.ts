...
function reflectionRepairObjective(request: EvolutionRequest, proposal: string, error: EvolutionError): string {
  return [
    ...
    "Tool output from one successful run is observational evidence, not automatically a durable output requirement. Treat exact output as normative only when the user or an authoritative repository contract explicitly requires it; never rewrite an existing verifier or tool merely to reproduce historical output.",
